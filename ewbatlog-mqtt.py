...
class MyDelegate(DefaultDelegate):
    ...
    def publishDataMqtt(self):
        payload = {
            ...
            "Volts": self.voltage / 100,
            "Amps": self.current / 100,
            "SOC_Ah": self.ahrem / 100,
            "Max_Ah": self.ahmax / 100,
            "Watts": self.watts,
            "SOC_pct": self.soc,
            "Temp": self.temp
        }
...
def connect_with_retry(addr, tries=10, delay=1.0):
    ...
    for i in range(tries):
        try:
            print("Connecting...")
            p = Peripheral()
            p.connect(addr, addrType=ADDR_TYPE_PUBLIC)
            print(f"Connected to {addr} public")
            return p
